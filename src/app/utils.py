...
def make_scatter(data, cluster=False, size=False):
    ...
    plt.hlines(y=txCorRacaPreta,
               ...
               xmax=data['txGenFeminino'].max(), 
               colors='black', 
               alpha=0.6, 
               linestyles='--', 
               label=f'Taxa Pretos: {100*txCorRacaPreta:.2f}%')
    
    plt.vlines(x=txGenFeminino, 
               ymin=data['txCorRacaPreta'].min(),
               ymax=data['txCorRacaPreta'].max(),
               colors='tomato',
               alpha=0.6, 
               linestyles='--', 
               label=f'Taxa Mulheres: {100*txGenFeminino:.2f}%')
